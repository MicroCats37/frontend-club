...
import { BungalowPriceDetailsModal } from "./BungalowPriceDetailsModal";
...
const _DIAS_LABELS: Record<number, string> = {
	1: "L",
	2: "M",
	3: "X",
	4: "J",
...
export function BungalowPricingList() {
	...
	const [viewingBungalow, setViewingBungalow] = useState<any | null>(null);
		...
		return (
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{[1, 2, 3].map((i) => (
					<Card
						key={i}
						className="rounded-[2.5rem] border-none shadow-sm overflow-hidden h-[350px]"
					>
		...
		return (
			<div className="p-16 text-center bg-red-50/50 text-red-600 rounded-[3rem] border-2 border-dashed border-red-100 ring-8 ring-red-50/20">
				...
				<p className="text-sm font-medium opacity-70">
					No pudimos conectar con el servidor de precios.
				</p>
			</div>
		);
	}
		...
		return (
			<div className="p-16 text-center bg-gray-50/50 text-gray-400 rounded-[3rem] border-2 border-dashed border-gray-100 ring-8 ring-gray-50/10">
				...
				<p className="text-sm font-medium opacity-60">
					Parece que aún no hay bungalows registrados.
				</p>
			</div>
		);
	}
		...
		<div className="space-y-10">
			<div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-gray-100 pb-8 px-2">
				<div>
					...
					<h2 className="text-4xl font-black text-[#2C3A2C] tracking-tight">
						Precios por Unidad
					</h2>
					<p className="text-lg text-[#8BA18B] font-medium mt-1">
						Ajusta tarifas específicas para cada bungalow.
					</p>
				</div>
				<Badge
					variant="outline"
					className="bg-white border-gray-200 text-[#2C3A2C] px-5 py-2 rounded-2xl font-black text-sm shadow-sm ring-4 ring-gray-50"
				>
					{bungalows.length}{" "}
					<span className="ml-1 text-[#8BA18B] font-bold uppercase text-[10px]">
						Unidades
					</span>
				</Badge>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
				{bungalows.map((bungalow) => (
					<Card
						key={bungalow.id}
						className="rounded-[2.5rem] border-none shadow-sm hover:shadow-2xl hover:shadow-primary/5 transition-all duration-500 group overflow-hidden bg-white flex flex-col ring-1 ring-gray-100/50"
					>
						<div className="p-8 pb-4">
							...
							<div className="flex flex-wrap gap-2 mb-6">
								<Badge className="bg-gray-100 text-[#4A5D4A] border-none text-[10px] font-black rounded-xl px-3 py-1 uppercase tracking-wider">
									<MapPin className="w-3 h-3 mr-1 opacity-50" /> Zona{" "}
									{bungalow.zona || "A"}
								</Badge>
								<Badge className="bg-primary/5 text-primary border-primary/10 text-[10px] font-black rounded-xl px-3 py-1 uppercase tracking-wider">
									<Users className="w-3 h-3 mr-1 opacity-50" /> Cap.{" "}
									{bungalow.capacidad}
								</Badge>
							</div>
						</div>
						...
						<CardContent className="px-8 pt-0 flex-1">
							<div className="space-y-4">
								<div className=" mt-auto">
									<Button
										onClick={() => setViewingBungalow(bungalow)}
										className="w-full h-12 bg-[#064E3B] hover:bg-emerald-900 text-white rounded-xl font-black text-xs shadow-lg shadow-emerald-950/20 transition-all hover:scale-[1.02] flex items-center justify-center gap-2"
									>
			...
			{viewingBungalow && (
				<BungalowPriceDetailsModal
					bungalow={viewingBungalow}
					open={!!viewingBungalow}
					onOpenChange={(open) => !open && setViewingBungalow(null)}
				/>
			)}
		</div>
	);
