...
import { useSyncTarifasByCapacity, type TipoTarifa } from "@/hooks/useTarifas";
import { 
	Dialog, 
	DialogContent, 
	DialogHeader, 
...
export function SyncCapacidadModal({ open, onOpenChange, tipo }: SyncCapacidadModalProps) {
	const { data: bungalows, isLoading: loadingBungalows } = useBungalowPricingList();
	...
	const onSubmit = (data: any) => {
		if (!tipo || !tipo.reglas?.[0]?.id) return;

		const payload = {
			dias_tarifa_id: tipo.reglas[0].id,
			...
			<DialogContent className="sm:max-w-xl rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
				<DialogHeader className="p-8 bg-primary/5">
					...
					<DialogDescription className="text-[#8BA18B] text-lg font-medium">
						Configura precios para <span className="text-primary font-bold">"{tipo?.nombre}"</span>
						{tipo?.es_paquete && (
							<Badge variant="outline" className="ml-2 text-[10px] uppercase font-bold text-amber-600 bg-amber-50 border-amber-200">
								Paquete
							</Badge>
						)}
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className="p-8 max-h-[50vh] overflow-y-auto">
						{loadingBungalows ? (
							...
							<div className="p-6 bg-red-50 text-red-600 rounded-3xl border border-red-100 flex items-center gap-4">
								...
								<p className="font-bold">No se encontraron bungalows para sincronizar.</p>
							</div>
						) : (
							<table className="w-full">
								<thead>
									<tr className="border-b border-gray-100">
										<th className="text-left py-3 font-black text-[#2C3A2C]">Capacidad</th>
										<th className="text-left py-3 font-black text-[#2C3A2C]">S/ Con Priv.</th>
										<th className="text-left py-3 font-black text-[#2C3A2C]">S/ Sin Priv.</th>
									</tr>
								</thead>
								<tbody>
									{capacidadesUnicas.map((cap, index) => (
										<tr key={cap} className="border-b border-gray-50 group hover:bg-primary/[0.02]">
											<td className="py-4">
												<div className="flex items-center gap-2">
													...
													<input 
														...
														value={cap}
													/>
													<span className="font-bold text-[#2C3A2C]">{cap} Personas</span>
												</div>
											</td>
					...
					<DialogFooter className="p-8 bg-gray-50/50 flex flex-col sm:flex-row gap-3">
						...
						<Button
							type="submit"
							disabled={syncMutation.isPending || capacidadesUnicas.length === 0}
