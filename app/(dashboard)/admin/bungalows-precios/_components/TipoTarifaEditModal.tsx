...
import { type TipoTarifa, useUpdateTipoTarifa } from "@/hooks/useTarifas";
import {
...
interface PricingItem {
	capacidad: number;
	precios: {
		noche_con_priv: number | string;
		noche_sin_priv: number | string;
		paquete_con_priv?: number | string;
		paquete_sin_priv?: number | string;
	};
}
...
export function TipoTarifaEditModal({
	...
	// Sync con tipo cuando cambia
	useEffect(() => {
		if (tipo && open) {
			// Hidratar reglas desde el tipo
			const hydratedReglas = ((tipo.reglas as any[]) || []).map((r) => ({
				nombre: r.nombre || "Regla",
				motor: r.motor || "DIAS_SEMANA",
				config: {
					fechas: r.config?.fechas || [],
					desde: r.config?.desde || "",
					hasta: r.config?.hasta || "",
					dias: r.config?.dias || [],
				},
				es_paquete_obligatorio: r.es_paquete_obligatorio || false,
				precios_capacidad:
					(r.precios_capacidad as any[])?.map((p) => ({
						capacidad: p.capacidad,
						precios: {
							noche_con_priv:
								p.precios.noche_con_priv !== 0 &&
								p.precios.noche_con_priv !== "0"
									? p.precios.noche_con_priv?.toString() || ""
									: "",
							noche_sin_priv:
								p.precios.noche_sin_priv !== 0 &&
								p.precios.noche_sin_priv !== "0"
									? p.precios.noche_sin_priv?.toString() || ""
									: "",
							paquete_con_priv:
								p.precios.paquete_con_priv !== 0 &&
								p.precios.paquete_con_priv !== "0"
									? p.precios.paquete_con_priv?.toString() || ""
									: "",
							paquete_sin_priv:
								p.precios.paquete_sin_priv !== 0 &&
								p.precios.paquete_sin_priv !== "0"
									? p.precios.paquete_sin_priv?.toString() || ""
									: "",
						},
					})) ||
					capacities.map((c: number) => ({
						capacidad: c,
						precios: {
							noche_con_priv: "",
							noche_sin_priv: "",
			...
			setReglas(hydratedReglas.length > 0 ? hydratedReglas : []);
		}
	...
	const handleUpdate = async (values: TipoTarifaInput) => {
		if (!tipo) return;
				...
				data: {
					...
					reglas: reglas.map((r) => ({
						...
						config: {
							...r.config,
							dias:
								r.motor === "DIAS_SEMANA" || r.motor === "RANGO"
									? r.config.dias
									: undefined,
					...
					initialData={
						tipo
							? {
									nombre: tipo.nombre,
									activo: tipo.activo,
									image_main: tipo.image_main,
									reglas: [], // Se maneja por estado aparte por ahora
								}
							: undefined
					}
							...
							<>
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 bg-gray-50/30 p-4 sm:p-6 rounded-3xl border border-gray-100">
											<div className="space-y-6">
												<div className="space-y-2">
													...
													<Input
														placeholder="Ej: Verano 2026 - Preventa"
											...
											<div className="space-y-4">
												<div
													className={`p-4 rounded-2xl border transition-all flex items-center justify-between ${methods.watch("activo") ? "bg-green-50 border-green-200 shadow-sm" : "bg-gray-50/50 border-gray-100"}`}
												>
													<div className="flex gap-3 items-center">
														<div
															className={`h-10 w-10 rounded-xl flex items-center justify-center ${methods.watch("activo") ? "bg-green-100 text-green-600" : "bg-white text-gray-400"}`}
														>
															<Badge className="p-0 border-none bg-transparent">
																<div
																	className={`h-2.5 w-2.5 rounded-full ${methods.watch("activo") ? "bg-green-500 animate-pulse" : "bg-gray-300"}`}
																/>
															</Badge>
														</div>
														<div className="flex flex-col">
															<Label
																className={`font-bold ${methods.watch("activo") ? "text-green-900" : "text-gray-700"}`}
															>
												...
												<div className="text-[10px] text-[#8BA18B] bg-white p-3 rounded-xl border border-gray-100 italic">
													* Editar esta configuración afectará a todos los
													bungalows que usen esta categoría de tarifa.
												</div>
											</div>
										</div>
																				...
																				<div className="space-y-3">
																					...
																					<div className="flex flex-wrap gap-2 max-h-[150px] overflow-y-auto p-1">
																						...
																						{rule.config.fechas.map((f) => (
																							<Badge
																								key={f}
																								className="bg-primary/5 text-primary border-primary/20 rounded-lg px-2 py-1 flex items-center gap-1 group"
																							>
																								{f}
																								<Trash2
																									className="h-3 w-3 cursor-pointer opacity-50 group-hover:opacity-100 hover:text-red-500"
																									onClick={() =>
																										updateRule(rIdx, {
																											config: {
																												...rule.config,
								...
								<DialogFooter className="p-6 sm:p-10 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row gap-4 sm:items-center shrink-0">
									...
									>
										...
										{isSubmitting ? "Actualizando..." : "Confirmar Tarifario"}
									</Button>
								</DialogFooter>
							</>
						);
