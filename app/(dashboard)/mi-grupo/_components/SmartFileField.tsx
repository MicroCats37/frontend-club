  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center px-1">
        <Label className={`font-bold text-[#4A5D4A] ${error ? "text-destructive" : ""}`}>{label}</Label>
        {isExistingUrl && (
      ...
      {isExistingUrl && (
        <div className="flex items-center gap-3 p-3 border rounded-md bg-[#F8FAF8] border-[#E0E7E0]">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-bold text-[#2C3A2C] truncate whitespace-nowrap overflow-hidden">DNI Registrado</p>
            <p className="text-xs text-muted-foreground italic">Documento verificado anteriormente</p>
          </div>
          <Button
            type="button"
            variant="ghost"
      ...
      {error && <p className="text-sm font-medium text-destructive px-1">{error.message}</p>}
    </div>
  );
};
