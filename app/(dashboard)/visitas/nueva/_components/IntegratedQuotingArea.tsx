					...
					<div className="flex items-center gap-5">
						...
						<div>
							...
							<p className="text-sm font-medium text-muted-foreground flex items-center gap-2">
								...
								{guestSelections.length} {guestSelections.length === 1 ? 'Invitado' : 'Invitados'} • 
								{fechas.start ? format(fechas.start, "d 'de' MMMM", { locale: es }) : 'Sin fecha'}
							</p>
						</div>
					</div>
					...
					<div className="flex items-center gap-4 w-full md:w-auto">
						{hasQuoted && (
							<div className="flex flex-col items-end px-6">
								<span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Total Estimado</span>
								...
								{cotizacion?.desglose.map((p, idx) => (
									...
									>
										<div className="flex items-center gap-4">
											...
											<div>
												<p className="font-extrabold text-[#2C3A2C]">{p.nombre_completo}</p>
												<div className="flex items-center gap-3">
													<span className="text-xs font-semibold text-muted-foreground">Pase General</span>
													{p.usa_cupon && (
										...
										<div className="text-right">
											<p className="text-sm font-black text-[#2C3A2C]">S/ {Number(p.total_persona || 0).toFixed(2)}</p>
											<p className="text-[10px] font-bold text-muted-foreground">Precio Unitario</p>
										</div>
									</div>
								))}
							</div>
							...
							<div className="mt-10 p-6 bg-primary/5 rounded-[32px] border border-primary/10 flex flex-col md:flex-row items-center justify-between gap-6">
								<div className="flex items-center gap-4">
									...
									<div className="text-center md:text-left">
										<h5 className="font-black text-[#2C3A2C]">¿Tienes un cupón promocional?</h5>
										<p className="text-xs font-medium text-muted-foreground">Podrás aplicarlo en el resumen final de pago.</p>
									</div>
								</div>
