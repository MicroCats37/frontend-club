...
export const ReceiptModal = ({
	...
	if (visita.reserva_asociada) {
		...
		ordenes.push({
			...
			saldo_pendiente:
				oc?.saldo_pendiente ?? (res.esta_pagada ? 0 : (res.precio_total ?? 0)),
	...
	if (visita.lista_ingresantes) {
		...
		ordenes.push({
			...
			saldo_pendiente:
				oc?.saldo_pendiente ??
				(lista.esta_pagada ? 0 : (lista.monto_total ?? 0)),
				...
				<div className="p-8 space-y-8 bg-white max-h-[80vh] overflow-y-auto no-scrollbar print:max-h-none print:p-12">
					...
					<div className="grid grid-cols-2 gap-8">
						...
						<div className="space-y-2 text-right">
							...
							<Badge
								className={`${isBungalow ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"} border-none rounded-lg px-3 py-1 font-black text-[10px]`}
							>
					...
					<div className="space-y-4">
						...
						<div className="space-y-3">
							{ordenes.map((orden, idx) => (
								<div
									key={idx}
									className="bg-gray-50 p-5 rounded-2xl border border-gray-100/50 flex flex-col gap-4"
								>
									<div className="flex justify-between items-center">
										<div className="flex items-center gap-3">
											<div
												className={`${orden.tipo === "ALOJAMIENTO" ? "bg-amber-100 text-amber-600" : "bg-emerald-100 text-emerald-600"} p-2 rounded-xl`}
											>
												{orden.tipo === "ALOJAMIENTO" ? (
									...
									<div className="flex justify-between items-end border-t border-gray-100 pt-3 mt-1">
										<div className="space-y-1">
											...
											<p className="font-black text-xl text-gray-900">
												S/ {parseNum(orden.monto_total).toFixed(2)}
											</p>
										</div>
										<div className="text-right space-y-1">
											...
											<p
												className={`font-black text-xl ${parseNum(orden.saldo_pendiente) > 0 ? "text-rose-500" : "text-emerald-500"}`}
											>
										...
										<div className="bg-rose-50 border border-rose-100 p-4 rounded-xl flex items-center gap-3">
											...
											<div>
												<p className="text-xs font-black text-rose-700 uppercase tracking-tight">
													ATENCIÓN: ORDEN VENCIDA
												</p>
												<p className="text-[10px] text-rose-600 font-medium">
													El plazo de pago ha expirado. Comuníquese con
													administración.
												</p>
											</div>
										</div>
